    ...
    D: Deserializer<'de>,
{
    ...
    i32::from_str_radix(&s[2..], 16)
        .map_err(|e| serde::de::Error::custom(format!("Failed to parse hex string: {}", e)))
}
...
pub fn from_u64_opt_hex_str<'de, D>(deserializer: D) -> Result<Option<u64>, D::Error>
where
    D: Deserializer<'de>,
{
    let opt: Option<String> = Deserialize::deserialize(deserializer)?;
    if let Some(hex_str) = opt {
        u64::from_str_radix(hex_str.trim_start_matches("0x"), 16)
            .map(Some)
            .map_err(|e| de::Error::custom({ format!("Failed to parse hex string: {}", e) }))
    } else {
        Ok(None)
    }
}
    ...
    D: Deserializer<'de>,
{
    ...
    let timestamp_u32 = from_i32_hex_str(deserializer).map_err(|e| {
        serde::de::Error::custom(format!("Invalid hex string for timestapm: {}", e))
    })?;

    Utc.timestamp_opt(timestamp_u32.into(), 0)
        .single()
        .ok_or_else(|| serde::de::Error::custom("Failed to convert timestamp to DateTime"))
}
...
mod tests {
    ...
    fn from_i32_opt_hex_str_test() {
        ...
        let result: TestStructWith_i32_opt = serde_json::from_value(data).unwrap();
        assert_eq!(
            result.value,
        ...
        let json_obj = serde_json::from_value::<TestStructWith_i32_opt>(json_value).unwrap();
    ...
    fn from_u32_hex_str_test() {
        ...
        let json_obj = serde_json::from_value::<TestStructWith_u32>(json_value).unwrap();
    ...
    fn from_u64_hex_str_test() {
        ...
        let json_obj = serde_json::from_value::<TestStructWith_u64>(json_value).unwrap();
    ...
    fn from_u128_hex_str_test() {
        ...
        let data = json!({"id": "7", "value": "0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF"});
        let result: TestStructWith_u128 = serde_json::from_value(data).unwrap();
        assert_eq!(
            result.value,
            u128::from_str_radix("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF", 16).unwrap()
        );

        let obj = TestStructWith_u128 {
            id: "7".to_string(),
            value: u128::from_str_radix("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF", 16).unwrap(),
        };
    ...
    fn from_unix_timestamp_hex_str_test() {
        ...
        let result = serde_json::from_value::<TestStructWith_ts_hex>(invalid_data);
